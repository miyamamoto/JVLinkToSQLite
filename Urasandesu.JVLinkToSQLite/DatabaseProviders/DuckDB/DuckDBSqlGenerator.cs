...
{
    /// <summary>
    /// Generates DuckDB-specific SQL statements.
    /// </summary>
    /// <remarks>
    /// DuckDB is optimized for OLAP (analytics) workloads with stronger type support than SQLite.
    /// Key differences from SQLite:
    /// - Uses BIGINT for long integers (vs INTEGER)
    /// - Uses TIMESTAMP for DateTime (vs TEXT)
    /// - Uses BOOLEAN for bool (vs INTEGER)
    /// - Uses DECIMAL(18,6) for decimal (vs REAL)
    /// </remarks>
    public class DuckDBSqlGenerator : ISqlGenerator
    {
        ...
        public string MapCSharpTypeToSqlType(Type csharpType)
        {
            ...
            throw new NotSupportedException($"Type '{csharpType.FullName}' is not supported for DuckDB mapping.");
        }
