...
{
    [TestFixture]
    public class DuckDBSqlGeneratorTests
    {
        private DuckDBSqlGenerator _generator;

        [SetUp]
        public void SetUp()
        {
            _generator = new DuckDBSqlGenerator();
        }

        [Test]
        [Category("Unit")]
        [Category("DuckDB")]
        public void DatabaseType_ReturnsDuckDB()
        {
            // Assert
            Assert.That(_generator.DatabaseType, Is.EqualTo(DatabaseType.DuckDB));
        }
        ...
        [Category("Unit")]
        [Category("DuckDB")]
        public void MapCSharpTypeToSqlType_String_ReturnsVarchar()
        {
            // Act
            var result = _generator.MapCSharpTypeToSqlType(typeof(string));

            // Assert
            Assert.That(result, Is.EqualTo("VARCHAR"));
        }
        ...
        [Category("Unit")]
        [Category("DuckDB")]
        public void MapCSharpTypeToSqlType_Decimal_ReturnsDecimal()
        {
            // Act
            var result = _generator.MapCSharpTypeToSqlType(typeof(decimal));

            // Assert
            Assert.That(result, Is.EqualTo("DECIMAL(18,6)"));
        }

        [Test]
        [Category("Unit")]
        [Category("DuckDB")]
        public void MapCSharpTypeToSqlType_Double_ReturnsDouble()
        {
            // Act
        ...
        [Category("Unit")]
        [Category("DuckDB")]
        public void MapCSharpTypeToSqlType_NullableDateTime_ReturnsTimestamp()
        {
            // Act
            var result = _generator.MapCSharpTypeToSqlType(typeof(DateTime?));
        ...
        public void GenerateCreateTableDdl_ValidType_ReturnsValidDdl()
        {
            ...
            Assert.That(result, Does.Contain("VARCHAR"));
